...
provider "zitadel" {
  ...
  # token            = data.local_file.access_token.content
  # token = data.local_file.access_token.filename
  jwt_profile_file = local.access_token_file
}
...
locals {
  access_token_file = "${var.state_dir}/devenv-cli.token"
  app_key_path      = "${var.state_dir}/devenv-app-key.json"
}
...
resource "zitadel_login_policy" "github_login_policy" {
  ...
  user_login                    = true
  allow_register                = true
  default_redirect_uri          = var.redirect_uris[0]
...
resource "local_file" "login_client_pat_file" {
  ...
  filename = "${var.state_dir}/login-client.pat"
}
...
# Output the OIDC client ID and other important values
output "project_id" {
